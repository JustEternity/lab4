...
class TaskWindow(QtWidgets.QDialog):
    ...
    def save_task(self):
        ...
        if not name:
            QtWidgets.QMessageBox.warning(self, "Ошибка", "Введите название задачи!") # Встроенное в PyQT диалоговое окно подтверждения
            return

        # Получаем дату из DateTimeEdit
        date_time = self.ui.dateTimeEdit.dateTime()
        formatted_date = date_time.toString("dd-MM-yyyy HH:mm") # Форматирует дату для одинакового вывода
    ...
    def delete_task(self):
        """Удаляет задачу"""
        if self.is_edit_mode:
            reply = QtWidgets.QMessageBox.question( # Встроенное в PyQT диалоговое окно подтверждения
                self, 'Подтверждение',
...
class MainWindow(QtWidgets.QDialog):
    ...
    def show_context_menu(self, position):
        ...
        if item:
            self.context_menu.exec(self.ui.listtask.mapToGlobal(position)) # преобразует локальные координаты виджета в глобальные (от экрана)
